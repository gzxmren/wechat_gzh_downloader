...
### 🛠️ 故障修复 (Bug Fixes)
- **根除 BeautifulSoup 崩溃**: 修复了在处理某些特殊文章时，因正文提取失败导致传递 `None` 给 `BeautifulSoup` 而引发的 `TypeError: Incoming markup is of an invalid type: None` 报错。通过在解析器和转换器层增加双重防御性校验，确保了程序的稳健运行。

### ✨ 功能增强 (Features)
- **适配 2026 新版混合布局**: 全面支持微信公众号新推出的“图片/文本混合频道”文章结构。
  - **JS 渲染正文提取**: 攻克了正文隐藏在 JS `JsDecode` 混淆字段中的难题，实现了对 `content_noencode` 数据的精准抓取。
  - **增强型图片频道解析器**: 升级了 `ImageDetailParser`，新增对 `picture_page_info_list`（冒号写法）和 `album_info_list` 的支持。
  - **鲁棒性解析策略**: 采用“文字优先，图片互补”的混合解析逻辑，大幅提升了新型文章的抓取成功率。
