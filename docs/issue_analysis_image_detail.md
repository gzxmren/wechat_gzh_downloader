...
## 9. 文章摘要/导语提取优化 (2026-01-23)

### 问题现象
部分图片频道文章除了图片外，还包含一段总体的导语文字。原解析器仅关注图片列表，导致这段文字丢失。

### 解决方案
1.  **Meta 标签提取**：在 HTML 源码中查找 `<meta name="description" content="...">` 标签。对于此类特殊页面，微信通常将正文文本存储于此。
2.  **内容格式化**：由于 meta content 中的换行符通常以 `
` 形式存在，解析时将其自动转换为 HTML 的 `<br/>` 标签以保持排版。
3.  **置顶显示**：在重构的 HTML 内容中，将提取到的描述文字插入到图片列表上方，确保导语位置正确。
